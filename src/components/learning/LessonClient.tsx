    ...
    <div className="flex flex-col h-full bg-white">
      <div className="shrink-0 px-5 py-2 flex items-center gap-3 border-b border-gray-100">
        ...
        <div className="relative flex items-center gap-1 shrink-0">
          ...
          {xpPop && <span className="pointer-events-none absolute -top-7 right-0 text-indigo-500 font-bold text-xs whitespace-nowrap animate-xp-pop">+10 XP</span>}
        </div>
      </div>
...
function LessonContent({ lesson, roadmapId }: { lesson: Lesson; roadmapId: string }) {
  ...
  useEffect(() => {
    ...
    setStreaming(true)
    fetch('/api/lesson', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lessonId: lesson.id }) })
      .then(async res => {
        ...
        while (true) {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              try { const data = JSON.parse(line.slice(6)); if (data.done && data.sections) setSections(data.sections) } catch {}
            }
          }
        }
        setStreaming(false)
  ...
  if (streaming && sections.length === 0) return (
    <div className="flex flex-col items-center justify-center h-96 gap-4">
      ...
      <div className="flex gap-1.5">
        {[0, 1, 2].map(i => <div key={i} className="w-2 h-2 bg-indigo-200 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
      </div>
    </div>
  )
  ...
  if (done) return (
    <div className="flex flex-col items-center justify-center h-full py-20 gap-6 text-center px-6">
      ...
      <button onClick={() => router.push(`/learn/${roadmapId}`)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-semibold text-sm transition-colors">Back to course</button>
    </div>
  )

  const lines = buildLines(sections)
  return <SpotifyReader lines={lines} lessonId={lesson.id} onFinish={() => { setXpEarned(Math.max(10, Math.floor(lines.length / 8) * 10)); setDone(true) }} />
}
    ...
    return (
      <div>
        ...
        <div className="space-y-4">
          {questions.map((q, i) => { const correct = answers[q.id] === q.correctIndex; return (
            <div key={q.id} className={cn('p-4 rounded-2xl border', correct ? 'border-green-100 bg-green-50' : 'border-red-100 bg-red-50')}>
              ...
              <div className="space-y-1.5">{q.options.map((opt, oi) => <div key={oi} className={cn('text-xs px-3 py-2 rounded-xl', oi === q.correctIndex && 'bg-green-100 text-green-700 font-medium', oi === answers[q.id] && oi !== q.correctIndex && 'bg-red-100 text-red-600', oi !== q.correctIndex && oi !== answers[q.id] && 'text-gray-400')}>{opt}</div>)}</div>
    ...
    <div className="space-y-5">
      {questions.map((q, i) => (
        <div key={q.id} className="p-5 bg-gray-50 border border-gray-100 rounded-2xl">
          ...
          <div className="space-y-2">{q.options.map((opt, oi) => <button key={oi} onClick={() => setAnswers(prev => ({ ...prev, [q.id]: oi }))} className={cn('w-full text-left text-sm px-4 py-3 rounded-xl border transition-all', answers[q.id] === oi ? 'border-indigo-400 bg-indigo-50 text-indigo-700 font-medium' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300')}>{opt}</button>)}</div>
        </div>
      ))}
...
function AssignmentSection({ lessonId, existingAssignment }: { lessonId: string; existingAssignment: Assignment | null }) {
  ...
  const generateAssignment = async () => { setLoading(true); const res = await fetch('/api/assignment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lessonId }) }); setAssignment(await res.json()); setLoading(false) }
  const submitAssignment = async () => {
    if (!assignment || !submission.trim()) return; setStreaming(true); setFeedback('')
    ...
    while (true) { const { done, value } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); const lines = buffer.split('\n\n'); buffer = lines.pop() ?? ''; for (const line of lines) { if (line.startsWith('data: ')) { try { const data = JSON.parse(line.slice(6)); if (data.chunk) setFeedback(prev => prev + data.chunk); if (data.done) setScore(data.score) } catch {} } } }
    setStreaming(false)
  }
  ...
  return (
    <div className="space-y-4">
      <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-2xl"><h3 className="font-semibold text-sm text-indigo-700 mb-2">Your Task</h3><p className="text-sm text-gray-700 leading-relaxed">{assignment.prompt}</p></div>
      {!assignment.submission && <div><textarea value={submission} onChange={e => setSubmission(e.target.value)} rows={8} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm text-gray-800 focus:outline-none focus:border-indigo-400 resize-none" placeholder="Write your response here..." /><button onClick={submitAssignment} disabled={streaming || !submission.trim()} className="mt-3 w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white py-3 rounded-2xl text-sm font-semibold flex items-center justify-center gap-2">{streaming ? <Loader2 className="w-4 h-4 animate-spin" /> : null}{streaming ? 'Getting feedback...' : 'Submit for AI Feedback'}</button></div>}
      {(feedback || assignment.ai_feedback) && <div className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm"><div className="flex items-center justify-between mb-3"><h3 className="font-semibold text-sm text-gray-900">AI Feedback</h3>{score !== null && <span className={cn('text-sm font-bold px-3 py-1 rounded-full', score >= 80 ? 'bg-green-50 text-green-600' : score >= 60 ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-500')}>{score}/100</span>}</div><div className="prose prose-sm prose-gray max-w-none text-gray-600"><ReactMarkdown>{feedback || assignment.ai_feedback || ''}</ReactMarkdown></div>{streaming && <span className="animate-pulse text-indigo-400 text-lg">▊</span>}</div>}
    </div>
  )
}
...
export default function LessonClient({ lesson, roadmap, roadmapId, existingQuiz, existingAssignment }: Props) {
  ...
  const tabs = [{ id: 'content', label: '📖 Lesson' }, { id: 'quiz', label: '🧠 Quiz' }, { id: 'assignment', label: '✍️ Practice' }] as const
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 shrink-0">
        <button onClick={() => router.push(`/learn/${roadmapId}`)} className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"><X className="w-4 h-4 text-gray-600" /></button>
      ...
      <div className="flex border-b border-gray-100 shrink-0 px-2">
        {tabs.map(({ id, label }) => <button key={id} onClick={() => setActiveTab(id)} className={cn('flex-1 py-3 text-xs font-semibold transition-all border-b-2 -mb-px', activeTab === id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-400 hover:text-gray-600')}>{label}</button>)}
      </div>
